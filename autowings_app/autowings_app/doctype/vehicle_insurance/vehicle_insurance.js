...
// create vehicle insurance if skip

console.log("Vehicle Insurance script initialized");
...
frappe.ui.form.on("Vehicle Insurance", {
    refresh: function(frm) {
        ...
        // Add custom button "Create Vehicle Insurance"
        frm.add_custom_button(__("Create Vehicle Insurance"), function() {
            // Create a dialog for input
            let d = new frappe.ui.Dialog({
                title: __("Create Vehicle Insurance"),
                fields: [
                    {
                        ...
                        onchange: function() {
                            ...
                            if (si) {
                                // Check if Vehicle Insurance exists
                                frappe.call({
                                    ...
                                    callback: function(res) {
                                        if (res.message.length > 0) {
                                            showError(__("Vehicle Insurance already exists for Sales Invoice {0}.", [si]));
                ...
                primary_action: async function(values) {
                    try {
                        // Validate for existing Vehicle Insurance
                        const insurance_check = await frappe.call({
                            method: "frappe.client.get_list",
                            args: {
                                doctype: "Vehicle Insurance",
                        ...
                        if (insurance_check.message.length > 0) {
                            showError(__("Vehicle Insurance already exists for Sales Invoice {0}.", [values.sales_invoice]));
                            return;
                        }

                        // Create Vehicle Insurance
                        const response = await frappe.call({
                            method: "autowings_app.autowings_app.doctype.vehicle_insurance.vehicle_insurance.create_vehicle_insurance_from_sales_invoice",
                            args: {
                        ...
                        if (response.message) {
                            frappe.set_route("Form", "Vehicle Insurance", response.message);
                            showSuccess(__("Vehicle Insurance {0} created successfully.", [response.message]));
                        }
                    } catch (err) {
                        console.error("Error creating Vehicle Insurance:", err);
                        showError(__("Failed to create Vehicle Insurance: {0}", [err.message]));
                    }
                    d.hide();
                },
                secondary_action_label: __("Cancel"),
