...
console.log("Vehicle Extended Warranty script initialized");
...
frappe.ui.form.on("Vehicle Extended Warranty", {
    refresh: function(frm) {
        ...
        // Add custom button "Create Vehicle Extended Warranty"
        frm.add_custom_button(__("Create Vehicle Extended Warranty"), function() {
            // Create a dialog for input
            let d = new frappe.ui.Dialog({
                title: __("Create Vehicle Extended Warranty"),
                fields: [
                    {
                        ...
                        onchange: function() {
                            ...
                            if (si) {
                                // Check if Vehicle Extended Warranty exists
                                frappe.call({
                                    ...
                                    callback: function(res) {
                                        if (res.message.length > 0) {
                                            showError(__("Vehicle Extended Warranty already exists for Sales Invoice {0}.", [si]));
                    ...
                    },
                    {
                        ...
                        get_query: function() {
                            return {
                                filters: {
                                    supplier_group: "Extended Warranty Group"
                                }
                            };
                        }
                    },
                ...
                primary_action: async function(values) {
                    try {
                        // Validate for existing Vehicle Extended Warranty
                        const warranty_check = await frappe.call({
                            method: "frappe.client.get_list",
                            args: {
                                doctype: "Vehicle Extended Warranty",
                        ...
                        if (warranty_check.message.length > 0) {
                            showError(__("Vehicle Extended Warranty already exists for Sales Invoice {0}.", [values.sales_invoice]));
                            return;
                        }

                        // Create Vehicle Extended Warranty
                        const response = await frappe.call({
                            method: "autowings_app.autowings_app.doctype.vehicle_extended_warranty.vehicle_extended_warranty.create_vehicle_extended_warranty_from_sales_invoice",
                            args: {
                        ...
                        if (response.message) {
                            frappe.set_route("Form", "Vehicle Extended Warranty", response.message);
                            showSuccess(__("Vehicle Extended Warranty {0} created successfully.", [response.message]));
                        }
                    } catch (err) {
                        console.error("Error creating Vehicle Extended Warranty:", err);
                        showError(__("Failed to create Vehicle Extended Warranty: {0}", [err.message]));
                    }
                    d.hide();
                },
                secondary_action_label: __("Cancel"),
