...
frappe.listview_settings['RTO Registration'] = {
    onload: function(listview) {
        console.log("RTO Registration list view onload function called"); // Debug log to confirm onload is triggered

        // Custom Button 1: Receive Documents from DTO
        console.log("Adding Receive Documents from DTO action");
        listview.page.add_actions_menu_item(__("Receive Documents from DTO"), function() {
            console.log("Receive Documents from DTO action triggered");
            frappe.call({
                method: 'frappe.client.get_list',
                args: {
                    doctype: 'RTO Registration',
...
listview.page.add_actions_menu_item(__("Submit Documents to DTO"), function() {
    ...
    // Filter documents: Check status, registration_status, registration_number, and document_submitted_to_dto
    let filtered_docs = selected_docs.filter(doc => {
        // Base condition: status must be "Due Documents Submission to DTO"
        if (doc.status !== 'Due Documents Submission to DTO') {
            return false;
        }
        // Additional condition: document_submitted_to_dto must be 0
        if (doc.document_submitted_to_dto === 1) {
            return false;
        }
        // Additional condition: if registration_status exists, check it along with registration_number
        if (doc.registration_status === 'Due Documents Submission to DTO') {
            return doc.registration_number && doc.registration_number.trim() !== '';
        }
        // If registration_status is not "Due Documents Submission to DTO", only status and document_submitted_to_dto matter
        return true;
    });
    ...
    if (filtered_docs.length === 0) {
        frappe.msgprint({
            title: __('No Eligible Documents'),
            message: __('None of the selected documents meet the criteria for submission: status "Due Documents Submission to DTO", document not yet submitted, and (if applicable) registration number present.'),
            indicator: 'orange'
        });
        return;
    }

    console.log("Filtered Documents (status = Due Documents Submission to DTO, document_submitted_to_dto = 0, registration checks):", filtered_docs);
    ...
    Promise.all(promises)
        .then(docs => {
            ...
            docs.forEach(doc => {
                ...
                let is_eligible = doc.status === 'Due Documents Submission to DTO' && doc.document_submitted_to_dto === 0;
                if (doc.registration_status === 'Due Documents Submission to DTO') {
                    is_eligible = is_eligible && doc.registration_number && doc.registration_number.trim() !== '';
                }
            ...
            if (not_eligible_docs.length > 0) {
                ...
                not_eligible_docs.forEach(doc => {
                    ...
                    if (!due_accounts.length) {
                        dialog_html += 'Reason: ';
                        if (doc.document_submitted_to_dto === 1) {
                            dialog_html += 'Documents already submitted to DTO<br>';
                        } else if (doc.registration_status === 'Due Documents Submission to DTO' && (!doc.registration_number || doc.registration_number.trim() === '')) {
                            dialog_html += 'Registration number missing<br>';
                        } else {
        ...
        listview.page.add_actions_menu_item(__("Order Number Plates"), function() {
            console.log("Order Number Plates action triggered");
            frappe.call({
                method: 'frappe.client.get_list',
                args: {
                    doctype: 'RTO Registration',
                    filters: { status: 'Due Number Plate Ordering', number_plate_ordered: 0 },
                    fields: ['name', 'customer', 'chassis_number', 'modified'],
                    limit_page_length: 0
                },
                callback: function(response) {
                    const eligible_docs = response.message || [];
                    console.log("Eligible Documents (status = Due Number Plate Ordering):", eligible_docs);

                    if (eligible_docs.length === 0) {
                        frappe.msgprint({
                            title: __('No Eligible Documents'),
                            message: __('No RTO Registration documents with status "Due Number Plate Ordering" found.'),
                            indicator: 'orange'
                        });
                        return;
                    }
                    ...
                    let dialog_html = '<div style="margin-bottom: 20px;">';
                    dialog_html += '<h4 style="color: green;">' + __('Eligible for Number Plate Ordering') + '</h4>';
                    dialog_html += `
                        <div style="max-height: 200px; overflow-y: auto; border: 1px solid #28a745; padding: 10px; margin-bottom: 20px;">
                            <table style="width: 100%; border-collapse: collapse;">
                                <thead>
                    ...
                    let dialog = new frappe.ui.Dialog({
                        ...
                        primary_action: function(values) {
                            ...
                            console.log("Checked Documents for Number Plate Ordering:", checked_docs);

                            if (checked_docs.length === 0) {
                                frappe.msgprint({
                                    title: __('No Documents Selected'),
                                    message: __('Please select at least one document to order number plates.'),
                                    indicator: 'orange'
                                });
                                return;
                            }

                            const order_doc = (doc_name, retry_count = 0, max_retries = 3) => {
                                let doc = eligible_docs.find(d => d.name === doc_name);
                                if (!doc) {
                                    console.log(`Document ${doc_name} not found in eligible_docs`);
                                    return;
                                ...
                                console.log(`Ordering number plate for document: ${doc_name}`);
                                frappe.call({
                                    method: 'frappe.client.set_value',
                                    args: {
                                        doctype: 'RTO Registration',
                                    ...
                                    callback: function(r) {
                                        if (!r.exc) {
                                            log_rto_activity(doc, 'Number Plate Ordered', 'Order Placed', values.order_details);
                                            frappe.msgprint({
                                                title: __('Success'),
                                                message: __('Number Plate ordered for {0}.', [doc.name]),
                                                indicator: 'green'
                                            });
                                        } else if (r.exc.includes('TimestampMismatchError') && retry_count < max_retries) {
                                            console.log(`TimestampMismatchError for ${doc_name}, retrying (${retry_count + 1}/${max_retries})`);
                                            frappe.call({
                                                ...
                                                callback: function(refresh_response) {
                                                    if (refresh_response.message) {
                                                        eligible_docs = eligible_docs.map(d => d.name === doc.name ? refresh_response.message : d);
                                                        order_doc(doc_name, retry_count + 1, max_retries);
                                                    }
                                                }
                                            });
                                        } else {
                                            log_rto_activity(doc, 'Number Plate Ordered', 'Order Failed', r.exc || JSON.stringify(r));
                                            frappe.msgprint({
                                                title: __('Error'),
                                                message: __('Error ordering number plate for {0}: {1}', [doc.name, r.exc || JSON.stringify(r)]),
                                                indicator: 'red'
                                            });
                                        }
                                    },
                                    error: function(err) {
                                        log_rto_activity(doc, 'Number Plate Ordered', 'Order Failed', err.message || JSON.stringify(err));
                                        frappe.msgprint({
                                            title: __('Error'),
                                            message: __('Error ordering number plate for {0}: {1}', [doc.name, err.message || JSON.stringify(err)]),
                                            indicator: 'red'
                                        });
                                    }
                                });
                            ...
                            checked_docs.forEach(doc_name => order_doc(doc_name));
                        ...
                        secondary_action: function() {
                            ...
                            dialog.$wrapper.find('.eligible-doc:checked').each(function() {
                                ...
                                let doc = eligible_docs.find(d => d.name === doc_name);
                                if (doc) {
                                    log_rto_activity(doc, 'Number Plate Order Cancelled', 'Order Cancelled', '');
                                }
                            });
                        }
                    });
        ...
        // Custom Button 4: Number Plate Received
        console.log("Adding Number Plate Received action");
        listview.page.add_actions_menu_item(__("Number Plate Received"), function() {
            console.log("Number Plate Received action triggered");
            frappe.call({
                method: 'frappe.client.get_list',
                args: {
                    doctype: 'RTO Registration',
                    filters: { status: 'Number Plate Not Received', number_plate_ordered: 1, number_plate_received: 0 },
                    fields: ['name', 'customer', 'chassis_number', 'modified'],
                    limit_page_length: 0
                },
                callback: function(response) {
                    const eligible_docs = response.message || [];
                    console.log("Eligible Documents (status = Number Plate Not Received):", eligible_docs);

                    if (eligible_docs.length === 0) {
                        frappe.msgprint({
                            title: __('No Eligible Documents'),
                            message: __('No RTO Registration documents with status "Number Plate Not Received" found.'),
                            indicator: 'orange'
                        });
                        return;
                    }
                    ...
                    let dialog_html = '<div style="margin-bottom: 20px;">';
                    dialog_html += '<h4 style="color: green;">' + __('Eligible for Number Plate Receipt') + '</h4>';
                    dialog_html += `
                        <div style="max-height: 200px; overflow-y: auto; border: 1px solid #28a745; padding: 10px; margin-bottom: 20px;">
                            <table style="width: 100%; border-collapse: collapse;">
                                <thead>
                    ...
                    let dialog = new frappe.ui.Dialog({
                        ...
                        primary_action: function(values) {
                            ...
                            console.log("Checked Documents for Number Plate Receipt:", checked_docs);

                            if (checked_docs.length === 0) {
                                frappe.msgprint({
                                    title: __('No Documents Selected'),
                                    message: __('Please select at least one document to confirm number plate receipt.'),
                                    indicator: 'orange'
                                });
                                return;
                            }
                            ...
                            const receive_doc = (doc_name, retry_count = 0, max_retries = 3) => {
                                ...
                                console.log(`Confirming number plate receipt for document: ${doc_name}`);
                                frappe.call({
                                    method: 'frappe.client.set_value',
                                    args: {
                                        doctype: 'RTO Registration',
                                    ...
                                    callback: function(r) {
                                        if (!r.exc) {
                                            log_rto_activity(doc, 'Number Plate Received', 'Receipt Confirmed', values.received_details);
                                            frappe.msgprint({
                                                title: __('Success'),
                                                message: __('Number Plate receipt confirmed for {0}.', [doc.name]),
                                                indicator: 'green'
                                            });
                                        } else if (r.exc.includes('TimestampMismatchError') && retry_count < max_retries) {
                                            console.log(`TimestampMismatchError for ${doc_name}, retrying (${retry_count + 1}/${max_retries})`);
                                            ...
                                            log_rto_activity(doc, 'Number Plate Received', 'Receipt Failed', r.exc || JSON.stringify(r));
                                            frappe.msgprint({
                                                title: __('Error'),
                                                message: __('Error confirming number plate receipt for {0}: {1}', [doc.name, r.exc || JSON.stringify(r)]),
                                                indicator: 'red'
                                            });
                                        }
                                    },
                                    error: function(err) {
                                        log_rto_activity(doc, 'Number Plate Received', 'Receipt Failed', err.message || JSON.stringify(err));
                                        frappe.msgprint({
                                            title: __('Error'),
                                            message: __('Error confirming number plate receipt for {0}: {1}', [doc.name, err.message || JSON.stringify(err)]),
                                            indicator: 'red'
                                        });
                                    }
                                });
                        ...
                        secondary_action: function() {
                            ...
                            dialog.$wrapper.find('.eligible-doc:checked').each(function() {
                                ...
                                let doc = eligible_docs.find(d => d.name === doc_name);
                                if (doc) {
                                    log_rto_activity(doc, 'Number Plate Receipt Cancelled', 'Receipt Cancelled', '');
                                }
                            });
                        }
                    });
